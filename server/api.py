...
def _run_generate_sync(
    *,
    ...
    if cached.exists():
        jlog(job_id, "cache_hit", time.perf_counter() - t0, path=str(cached))
        return cached, None
    ...
    jlog(job_id, "analyze", time.perf_counter() - t_an, disk_gb=disk_free_gb())
    ...
    jlog(job_id, "plan", time.perf_counter() - t_pl, entries=len(timeline_list))
    ...
    jlog(job_id, "execute", time.perf_counter() - t_ex)

    t_m = time.perf_counter()
    master(str(raw), str(final), target_lufs=lufs)
    jlog(job_id, "master", time.perf_counter() - t_m)
    ...
    jlog(job_id, "done", time.perf_counter() - t0, artifact=str(art))
...
async def generate(
    ...
    try:
        ...
        try:
            ...
            jlog(job_id, "error", time.perf_counter() - t_wall, err=str(e)[:800])
        ...
        touch_idle()
        jlog(job_id, "response", time.perf_counter() - t_wall)
        return resp
    finally:
