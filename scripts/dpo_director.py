...
def _build_kto_rows(jsonl_path: str, pq_pass_thresh: float = 7.0) -> list[dict]:
    """Build KTO single-label rows: each (prompt, plan, label) is one
    render; label=True when composite (PQ+CE)/2 >= threshold. Unlocks
    full render log without pair construction."""
    rows: list[dict] = []
    with open(jsonl_path) as f:
        for line in f:
            ...
            rows.append({
                "prompt": prompt,
                "completion": (plan if isinstance(plan, str)
                               else json.dumps(plan, indent=2)),
                "label": bool(c >= pq_pass_thresh),
                "composite": c,
            })
    return rows
...
def main():
    ...
    ap.add_argument("--loss-type", default="sigmoid",
                    choices=["sigmoid", "ipo", "dpop", "kto"],
                    help="DPO loss variant. sigmoid=vanilla, "
                         "ipo=non-saturating squared, dpop=positive "
                         "(prevents both-degrade), kto=binary pass/fail "
                         "(needs --kto-mode).")
    ap.add_argument("--pq-pass-thresh", type=float, default=7.0,
                    help="(KTO only) composite-PQ threshold for "
                         "pass/fail labelling.")
    args = ap.parse_args()
    ...
    # KTO path: single-thumbs-up/down labels, no pair construction.
    # Beats DPO on small / noisy reward data. Unlocks ALL render logs.
    if args.loss_type == "kto":
        rows = _build_kto_rows(args.jsonl, args.pq_pass_thresh)
        if not rows:
            sys.exit(f"[dpo_director] no KTO rows (pq_pass_thresh="
                     f"{args.pq_pass_thresh})")
        print(f"[dpo_director] KTO mode: {len(rows)} labeled rows "
              f"(thresh={args.pq_pass_thresh})")
    else:
        pairs = _build_pairs(args.jsonl, min_delta=args.min_delta)
        if not pairs:
            sys.exit(f"[dpo_director] no preference pairs "
                     f"(min_delta={args.min_delta})")
        pairs = pairs[: args.max_pairs]
        print(f"[dpo_director] {args.loss_type} mode: {len(pairs)} pairs "
              f"from {args.jsonl}")
    ...
    print(f"[dpo_director] {args.loss_type} LoRA saved at {args.out}")
